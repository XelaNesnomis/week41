...
console.log('The position of raspberry is: ' + position);


function findFruitPosition(fruitName) {
    const position = fruits.indexOf(fruitName);
    
    if (position !== -1) {
        return `The position of ${fruitName} is: ${position}`;
    } else {
        return `${fruitName} is not in the list.`;
    }
}
...
console.log(`Number of fruits with names longer than 8 characters: ${longFruits.length}`);
console.log('Fruits with names longer than 8 characters:', longFruits);
...
function listsAreEqual(list1, list2) {
    ...
    for (let i = 0; i < list1.length; i++) {
      ...
      const indexInList2 = list2Copy.indexOf(element);
  
      if (indexInList2 === -1) {
        return false;
      } else {
        list2Copy.splice(indexInList2, 1);
      }
    }
